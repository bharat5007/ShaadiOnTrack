...
class CRUDWeddingCore(CRUDBase[Budget]):
    ...
    def create_for_user(self, db: Session, user_id: int, obj_in: dict) -> Budget:
...
class CRUDBudgetCategory(CRUDBase[BudgetCategory]):
    ...
    def get_by_wedding(
        ...
        return (
            ...
            .filter(BudgetCategory.wedding_id == wedding_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class CRUDVendor(CRUDBase[Vendor]):
    ...
    def get_by_service_category(
        self, db: Session, service_category_id: int, skip: int = 0, limit: int = 100
    ) -> List[Vendor]:
        """Get all vendors for a specific service category."""
        return (
            db.query(Vendor)
            .filter(Vendor.service_categories == service_category_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def search_by_name(
        ...
        return (
            db.query(Vendor)
            .filter(Vendor.name.ilike(f"%{name}%"))
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class CRUDVendorMedia(CRUDBase[VendorMedia]):
    ...
    def get_by_vendor(
        ...
        return (
            ...
            .filter(VendorMedia.vendor_id == vendor_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
