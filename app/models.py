...
class Budget(Base):
    ...
    # Relationships
    budget_categories = relationship("BudgetCategory", back_populates="wedding", cascade="all, delete-orphan")
    budget_vendor_maps = relationship("BudgetVendorMap", back_populates="wedding", cascade="all, delete-orphan")
...
class BudgetCategory(Base):
    ...
    budget_vendor_maps = relationship("BudgetVendorMap", back_populates="budget_category", cascade="all, delete-orphan")
...
class BudgetVendorMap(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    budget_id = Column(Integer, ForeignKey("budget_categories.id", ondelete="CASCADE"), nullable=False, index=True)
    vendor_id = Column(Integer, ForeignKey("vendors.id", ondelete="CASCADE"), nullable=False, index=True)
    budget_id = Column(Integer, ForeignKey("budget.id", ondelete="CASCADE"), nullable=False, index=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
